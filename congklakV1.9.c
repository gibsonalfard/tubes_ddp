...
int openScore();
void saveScore();
void swap(highscore H1,highscore H2,int i,int j,FILE *fp);
int cekSwap(highscore H1,highscore H2,int choice);
void sortHighscore(int choice);
void showHighscore();
...
void swap(highscore H1,highscore H2,int i,int j,FILE *fp){
    fseek(fp,i*sizeof(highscore),SEEK_SET);
	fwrite(&H2,sizeof(highscore),1,fp);
	
	fseek(fp,j*sizeof(highscore),SEEK_SET);
	fwrite(&H1,sizeof(highscore),1,fp);
}

//modul tipe fungsi untuk menghasilkan algoritma boolean sebagai penentu dilakukanya modul swap
int cekSwap(highscore H1,highscore H2,int choice){
    if(choice==1){
        return strcmp((H2.name),(H1.name))<0;
    }else if(choice==2){
        return H2.bijiCollected>H1.bijiCollected;
    }else if(choice==3){
        return H2.numberOfShoot>H1.numberOfShoot;
    }else if(choice==4){
        return H2.numberOfTake<H1.numberOfTake;
    }
}
...
void sortHighscore(int choice){
	...
	fp = fopen(HIGHSCORE_FILENAME,"rb+");	//rb+ digunakan untuk melakukan read/ write di dalam file
	...
	do{
		...
		fseek(fp,i*sizeof(highscore),SEEK_SET);
		fread(&H1,sizeof(highscore),1,fp);
		
		while(!feof(fp)){
			fseek(fp,j*sizeof(highscore),SEEK_SET);
			fread(&H2,sizeof(highscore),1,fp);
			if(!feof(fp)){
				if(strcmp((H2.name),(H1.name))<0){
					fseek(fp,i*sizeof(highscore),SEEK_SET);
					fwrite(&H2,sizeof(highscore),1,fp);
	...
	}while(!feof(fp) || (i < j-3));		//i < j -3 adalah kondisi yang melakukan pengecekan 
										//apakah proses sorting sudah dilakukan untuk semua file atau belum
	fclose(fp);
}
...
void showHighscore(){
    ...
    do{
        ...
        if (pilihan!=5){
            sortHighscore(pilihan);   //data di-sort berdasarkan pilihan
            N=openScore();
